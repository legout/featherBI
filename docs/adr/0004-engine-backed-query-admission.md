...
This keeps admission aligned with the executing engine rather than maintaining a second SQL interpretation. It is supported-language validation, not a sandbox for hostile authored SQL or HTML. The pinned WASM capability gate is already established; [GitHub issue #1](https://github.com/legout/featherBI/issues/1) delivered the smallest production admission path originally required by the [runtime contract §6](../specs/runtime-contract-v1.md) (now a superseded historical specification; the [v2 specification](../specs/dashboard-project-and-runtime-v2.md) owns current behavior). This ADR does not approve a regex workaround, custom parser, or unreviewed dependency upgrade.
