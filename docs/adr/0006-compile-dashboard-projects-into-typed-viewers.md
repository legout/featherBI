...
The final v2 migration replaces runtime contract v1 rather than extending it in place. Transitional dual support is allowed only while staged implementation remains green. Recipient SQL stays ephemeral and bounded; Perspective is a result viewer/preset above DuckDB, not a second source engine; AG Grid Enterprise and custom component plugins are outside the accepted scope.
