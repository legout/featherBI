...
Components declare `interactionDimension` (click replaces, modifier-click adds/removes multi-select values, re-click clears), `brushDimension` (date brush sets a date-range filter), and table columns may opt into `dimension` bindings. Values only flow into compatible shared filters of the same dimension; with no compatible filter the selection stays local. Every interaction creates a coherent filter revision; stale results never render.
