...
test("canonical rows carry the designed boundaries", async () => {
 const rows = JSON.parse(await readFile(path.join(fixturesDir, "rows.json"), "utf8"));
 assert.equal(rows.length, expected.canonical.rowCount);
 assert.ok(rows.some((row) => row.sequence_number === "001"), "leading-zero identifier preserved");
 assert.ok(
  rows.some((row) => row.sequence_number === null),
  "unquoted null distinguished from quoted empty string",
 );
 assert.ok(expected.defaultWindow.anchor.rowIndicesOutside.length === 1, "one row outside the 30-day window");
});
...
test("expected.json metrics are internally consistent", () => {
 ...
 assert.equal(expected.join.allRows.matchedInspectionRows + expected.join.allRows.unmatchedInspectionRows, expected.allRows.metrics.records);
});

test("runtime.config.json is accepted by the shared validator", async () => {
 const config = JSON.parse(await readFile(path.join(fixturesDir, "runtime.config.json"), "utf8"));
 const result = validateConfig(config);
 assert.equal(result.ok, true);
 assert.deepEqual(
  result.value.data.sources.map((source) => source.id).sort(),
...
test("generated CSV parses back to the canonical rows with types preserved", async () => {
 ...
 assert.ok(lines.some((line) => line.includes(",\"001\",")), "sequence_number 001 survives a CSV round-trip");
});
